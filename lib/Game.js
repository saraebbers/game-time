...
module.exports = class Game {
  constructor(ctx) {
    ...
    this.gameOver = false;
    this.frog = [
      new Frog(100, 550, 40, 40, 'green', 'pink')
    ];
    this.obstacle = [
      ...
      new Obstacle(650, 355, 40, 80, 'brown', 'yellow', -1, 2 ), new Obstacle(795, 305, 40, 80, 'white', 'brown', -1, 1 ),]
  }
  ...
  handleKeyPress(e) {
    ...
    if (e.key === 'ArrowRight') {
      // this.frog.rArrowPressed = true;
      this.frog.forEach( frog => {
        frog.frogMove(frog);
      });
    } else if (e.key === 'ArrowLeft') {
      this.frog.lArrowPressed = true;
      this.frog.forEach( frog => {frog.frogMove(frog)} );
      console.log('ArrowLeft in handleKeyPress');


    } else if (e.key === 'ArrowDown') {
      this.frog.dArrowPressed = true;
      this.frog.forEach( frog => {frog.frogMove(frog)} );
      console.log('ArrowDown in handleKeyPress');


    } else if (e.key === 'ArrowUp') {
      this.frog.uArrowPressed = true;
      this.frog.forEach( frog => {frog.frogMove(frog)} );
      console.log('ArrowUp in handleKeyPress');
